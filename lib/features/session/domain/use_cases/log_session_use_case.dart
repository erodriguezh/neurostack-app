...
/// 3. Create session entity (Session.create)
/// 4. Persist session (SessionRepository.save)
///
...
class LogSessionUseCase {
  ...
  /// - Save operation fails (from SessionRepository)
  Future<Either<DomainFailure, Session>> execute(LogSessionParams params) async {
    ...
    final sessionResult = userResult
        .flatMap((user) => user.canLogSession(
              params.protocolId,
              currentTime: params.currentTime,
            ))
        .flatMap((_) => Session.create(
              id: params.sessionId,
    ...
    return switch (sessionResult) {
      ...
      Right(:final value) => (await _sessionRepository.save(value)).map((_) => value),
    };
  }
}
